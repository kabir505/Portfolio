...
def is_membership_test_on_list(structure):
    ...
    Skips efficient cases based on 'details' content.
    """
    return (
...
def is_queue_like_list_usage(structure):
    print("[RULE TEST] checking line", structure.get("line"))
    print("[RULE TEST] usage_context:", structure.get("usage_context"))
    print("[RULE TEST] details:", structure.get("details"))

    if "efficient — using deque" in structure.get("details", "").lower():
        print("⚠️ Detected as efficient (SHOULD NOT BE)")
    else:
        print("✅ Detected as inefficient (SHOULD SUGGEST)")
